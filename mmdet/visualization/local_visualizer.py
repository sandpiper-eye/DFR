...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: List['InstanceData'],
        ...
        # Guard against classes being saved as a single string in checkpoint
        ...
        if 'bboxes' in instances and instances.bboxes.sum() > 0:
            ...
            # ========== Ultralytics-style adaptive rendering ==========
            # 关键修复1：确保图像是 HWC 格式且内存连续
            if image.ndim == 3 and image.shape[0] == 3:
                image = image.transpose(1, 2, 0)
            image = np.ascontiguousarray(image)
            
            
            lw = max(round(sum(image.shape[:2]) / 2 * 0.003), 2)  # line width
            sf = lw / 3                                           # font scale
            ...
            for i, (bbox, label) in enumerate(zip(bboxes, labels)):
                ...
                # 3) Text size
                (w, h), _ = cv2.getTextSize(label_text, 0,
                                            fontScale=sf, thickness=tf)

                # 4) Label position: outside if space permits, else inside top
                outside = (y1 - h - 3) >= 0
                if outside:
                    label_y1 = y1 - h - 3
                    label_y2 = y1
                ...
                # 5) Label background (same color as bbox)
                cv2.rectangle(image, (label_x1, label_y1),
                              (label_x2, label_y2), color, -1, cv2.LINE_AA)

                # 6) White text
                text_y = label_y2 - 2
                cv2.putText(image, label_text, (label_x1, text_y),
                            0, sf, (255, 255, 255), thickness=tf,
