...
class YOLOV3Head(BaseDenseHead):
    ...
    def loss_by_feat(
        ...
        mlvl_anchors = self.prior_generator.grid_priors(
            featmap_sizes, dtype=pred_maps[0][0].dtype, device=device)
    ...
    def loss_by_feat_single(self, pred_map: Tensor, target_map: Tensor,
        ...
        # === 修复DeepSpeed FP16训练的dtype不匹配问题 ===
        # 原代码: neg_mask = neg_map.float()  # 默认创建FP32
        # 修复: 使用pred_map的dtype，自动匹配FP16/FP32
        neg_mask = neg_map.to(dtype=pred_map.dtype)
    ...
    def _get_targets_single(self, anchors: List[Tensor],
        ...
        # === 修复DeepSpeed FP16训练的dtype不匹配问题 ===
        # bbox_coder.encode()返回FP32（因为gt_bboxes是FP32），需要转换为target_map的dtype
        encoded_bboxes = self.bbox_coder.encode(
            sampling_result.pos_priors, sampling_result.pos_gt_bboxes,
            anchor_strides[sampling_result.pos_inds])
        target_map[sampling_result.pos_inds, :4] = encoded_bboxes.to(dtype=target_map.dtype)

        target_map[sampling_result.pos_inds, 4] = 1

        # === 修复DeepSpeed FP16训练的dtype不匹配问题 ===
        # 原代码: gt_labels_one_hot = F.one_hot(...).float()  # 默认创建FP32
        # 修复: 使用target_map的dtype，自动匹配FP16/FP32
        gt_labels_one_hot = F.one_hot(
            gt_labels, num_classes=self.num_classes).to(dtype=target_map.dtype)
